...
class AdaptiveCoverLetterGenerator:
    ...
    def _generate_full(self, jd: Dict, analysis: Dict, cv_facts: Dict) -> str:
        ...
        exp_years = cv_facts.get("years_experience", "") if cv_facts else ""
...
- Background: {cv_facts.get("summary", "Experienced professional")[:200] if cv_facts else "Experienced professional"}
- Top achievement: {cv_facts.get("key_achievements", [""])[0][:150] if cv_facts and cv_facts.get("key_achievements") else "Delivered successful projects"}
...
- {cv_facts.get("key_achievements", [""])[0][:100] if cv_facts and cv_facts.get("key_achievements") else "Proven track record"}
...
No buzzwords. Just the letter text:"""
    ...
    def _generate_truncated(self, jd: Dict, analysis: Dict, cv_facts: Dict, limit: int) -> str:
        ...
        achievement = cv_facts.get("key_achievements", [""])[0] if cv_facts and cv_facts.get("key_achievements") else ""
        ...
        template = f"Dear Hiring Manager, I am applying for the {role} position at {company}. With {cv_facts.get('years_experience', 'relevant') if cv_facts else 'relevant'} years of experience delivering {metric}, I am confident in my ability to contribute effectively. I look forward to discussing this opportunity. Sincerely,"
...
Rewrite:"""
    ...
    def _fallback_letter(self, jd: Dict, cv_facts: Dict) -> str:
        company = jd.get("company_name", "your company")
        role = jd.get("role_title", "the position")
        exp_years = cv_facts.get("years_experience", "relevant") if cv_facts else "relevant"

        achievement = cv_facts.get("key_achievements", [""])[0] if cv_facts and cv_facts.get("key_achievements") else "delivered key projects"
...
[Your Name]"""
    ...
    def validate_against_cv(self, text: str, cv_facts: Dict) -> List[str]:
        ...
        # REMOVED: Company name checking - job titles and company names in context 
        # of "applying for X at Y" are not factual claims about CV content
        # This was causing false positives like "Junior Project Manager at XAL"
